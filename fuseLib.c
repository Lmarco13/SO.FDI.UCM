...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    NodeStruct *node = myFileSystem.nodes[fi->fh]; //fi->fh número de inodo
...
static int my_read(const char * path, char * buf, size_t size, off_t offset, struct fuse_file_info * fi){
    ...
    // Comprobar offset no sale fuera de fichero
    ...
    while(bytes2Read) { // 
        ...
        for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < total); i++) {
...
static int my_unlink(const char *path){
    ...
    // Buscamos pat en el directorio del SF
        /// Directorio raiz
    if(strcmp(path, "/") == 0) {
    ...
    /// Rest of the world
    if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) != -1) {
        idxNode = myFileSystem.directory.files[idxDir].nodeIdx;       // idxNode = nodo-i del ficheros
        resizeNode(idxNode, 0); // Truncamos el fichero usando resizeNode
        myFileSystem.directory.files[idxNode].freeFile = true; // Marcar entrada del directorio como libre
        myFileSystem.directory.numFiles--;  // Decrementar el contador de ficheros del directorio
        myFileSystem.nodes[myFileSystem.directory.files[idxDir].nodeIdx]->freeNode = true;  // Marcar inodo como libre
        myFileSystem.numFreeNodes++;    // Incrementar el contador de inodos libres
        updateDirectory(&myFileSystem); // Actualizar directorio en disco virtual
        updateNode(&myFileSystem, idxNode, myFileSystem.nodes[idxNode]); // Actualizar inodo en disco virtual
        // ? Liberar la memoria del inodo y actualizar la tabla
...
struct fuse_operations myFS_operations = {
    ...
    .read       = my_read,                      // Read data on an opened file
    .unlink     = my_unlink,                    // Read
};
